...
def generate_strings(length=5):
    global pattern, all_char, XXXX, STOPSEARCH
...
def main():
    ...
    # run('c+++')
    # VERBOSE = True
    # run('ay+-as-sc-++-bfy-bc+ssc+yy+-+++b+++ac+cbs++y+y+++ya++ybb+faf-fys+fa+sc')
    ...
    for i in range(0, 50):
        ...
        y = r * 5
        while not y < r * 1.5:
            x, y = get_valid_program(seed, random.randint(1, 25))
        arr50.append((x, y))
    ...
    while True:
        ...
        for i in range(0, 5000):
            ...
            x = arr50[j][0]
            while True:
                x, y = get_valid_program(x, random.randint(1, 25))
...
def run(program):
    ...
    while i < len(program):
        ...
        if VERBOSE and not SEARCH:
            print(
                f"{step} : {app_cur_reg} : {cur_reg} : {i} : {program[i]} : {steps_per_reg} : {used_breaks} : {unused_breaks} -- {registers}")
    ...
    if VERBOSE and not SEARCH:
        print(
            f"{step} : {app_cur_reg} : {cur_reg} : {i} :   : {steps_per_reg} : {used_breaks} : {unused_breaks} -- {registers}")
    ...
    if not SEARCH:
        ...
        print_result(program, cur_reg, cur_reg_steps, org, pc, pc_dir, prev_reg, reg_dir, registers, step,
                     steps_per_reg, unused_breaks, used_breaks)
    else:
        if not VERBOSE:
            value = 63245986
            if registers.__contains__(value):
                ...
                STOPSEARCH = True
            return pow((registers[0] - 100), 2) + pow((registers[1] - 100), 2) + pow((registers[2] - 100), 2) + pow(
                (registers[3] - 100), 2)
    return None


def print_result(program, cur_reg, cur_reg_steps, org, pc, pc_dir, prev_reg, reg_dir, registers, step, steps_per_reg,
                 unused_breaks, used_breaks):
    ...
    if not x:
        if VERBOSE:
            ...
            print(
                f"{org['unused_breaks'] == unused_breaks} \tunused_breaks\t {org['unused_breaks']} \t\t {unused_breaks} ")
            ...
            print(
                f"{org['steps_per_reg'] == steps_per_reg} \tsteps_per_reg\t {org['steps_per_reg']} \t {steps_per_reg} ")
            print(
                f"{org['cur_reg_steps'] == cur_reg_steps} \tcur_reg_steps\t {org['cur_reg_steps']} \t\t\t {cur_reg_steps}")
        else:
            print('WRONG  : ', end='')
