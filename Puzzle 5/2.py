...
def main():
    ...
    VERBOSE = False
    run('++a+abbbbbbbbbbbbbbbbbycyyca')
    # run('c++')
    # run('c+++')
    # run('s++')
    # run('s++')
    ...
    #         seed = ""
    #         for i in range(0, 35):
    #             seed += all_char[random.randint(0, len(all_char))]
    #         run(seed)
    #         break
    #     except:
    #         pass
    ...
    #     arr_all_y = []
    #     arr_all = []
    #     for i in range(0, num_arr_all):
    #         j = math.floor(i / (num_arr_all / num_arr_tops))
    #         x = arr_tops[j][0]
    #         while True:
    #             x, y = get_valid_program(x, random.randint(1, 10))
    #             if not arr_all_p.__contains__(x) and not arr_all.__contains__(x):
    #                 break
    #         arr_all_p.append(x)
    #         arr_all_y.append(y)
    #         arr_all.append((x, y))
    #
    #     arr_tops = sorted(arr_all, key=lambda student: student[1])[0:num_arr_tops]
    #
    #     if min[1] > arr_tops[0][1]:
    #         min = arr_tops[0]
    #
    #     SEARCH = False
    #     print(min, arr_tops[0], arr_tops[-1], end=" ")
    #     run(arr_tops[0][0])
    #     SEARCH = True
    #
    #     if arr_tops[0][1] == 0 or STOPSEARCH:
    #         break
